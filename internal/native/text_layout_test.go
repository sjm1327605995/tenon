...
func TestAutoWrap(t *testing.T) {
	face := getTestFace(t)
	// 涓€涓緢闀跨殑鑻辨枃鍙ュ瓙锛宮axWidth 寰堝皬锛屽簲璇ヨ鎹㈣
...
func TestWordBreakBreakAll(t *testing.T) {
	...
	// break-all 涓嬫瘡涓瓧绗﹂兘鍙柇锛屽簲璇ヤ骇鐢熷琛?
	if result.lineCount <= 1 {
...
func TestCJKWrap(t *testing.T) {
	face := getTestFace(t)
	result := computeTextLayout("杩欐槸涓€涓緢闀跨殑涓枃鏂囨湰鐢ㄤ簬娴嬭瘯鎹㈣鍔熻兘", face, 100, WhiteSpaceNormal, WordBreakNormal, 0)
	if result.lineCount <= 1 {
