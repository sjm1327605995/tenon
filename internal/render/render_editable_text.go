...
import (
	...
	"github.com/sjm1327605995/tenon/internal/font"
...
func (r *RenderEditableText) ensureCursorVisible() {
	...
	face, err := font.GetFontFace(font.FontDescriptor{
		Family: font.FontFamilyDefault,
...
func (r *RenderEditableText) Paint(screen *ebiten.Image, offset Offset) {
	...
	// 绘制文本或 placeholder
	if displayText != "" {
		r.drawTextContent(screen, textOffset, displayText, r.TextColor)
	} else if r.placeholder != "" {
		r.drawTextContent(screen, textOffset, r.placeholder, r.placeholderColor)
	}
...
// drawTextContent 绘制指定文本内容，不修改 r.RenderText.Content。
func (r *RenderEditableText) drawTextContent(screen *ebiten.Image, offset Offset, content string, clr color.Color) {
	if content == "" {
		return
	}
	bounds := r.bounds
	...
	text.Draw(screen, joined, face.Face, op)
}
