...
func (a *Accordion) buildItems() {
	for i, item := range a.items {
		...
		title := NewText(item.Title).SetFontSize(14).SetColor(core.GetTheme().TextColor)
		chevron := NewText("▶").SetFontSize(12).SetColor(core.GetTheme().MutedForegroundColor)

		header.Add(title, NewView().SetFlexGrow(1), chevron)
