...
func ShadcnCalendar(year int, month time.Month, selected time.Time, onSelect func(time.Time)) engine.Widget {
	t := engine.GetTheme()
	...
	).AlignItems(engine.AlignCenter).JustifyContent(engine.JustifySpaceBetween).Paddingf(engine.EdgeInsetsOnly(0, 4, 0, 4))

	// Weekday labels
	weekdays := []string{"Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"}
	dayLabels := make([]engine.Widget, 7)
	for i, d := range weekdays {
		dayLabels[i] = widgets.Container(
			widgets.Text(d).Color(render.NewColorFrom(t.MutedForegroundColor)).FontSize(12),
		).W(40).H(36)
	}
	labelRow := widgets.Row(dayLabels...).AlignItems(engine.AlignCenter)
	...
	weekRows := make([]engine.Widget, 0, 6)
	for w := 0; w < 6; w++ {
		rowVisible := false
		dayCells := make([]engine.Widget, 7)
		for d := 0; d < 7; d++ {
			...
			if cellDay >= 1 && cellDay <= daysInMonth {
				...
				cell := widgets.Container(
					...
					).JustifyContent(engine.JustifyCenter).AlignItems(engine.AlignCenter),
				).W(40).H(36).JustifyContent(engine.JustifyCenter).AlignItems(engine.AlignCenter).Border(colorToRender(t.BorderColor), borderW).Radius(t.BorderRadius).OnTap(func() {
					if onSelect != nil {
				...
				dayCells[d] = widgets.Container(widgets.Text("")).W(40).H(36).JustifyContent(engine.JustifyCenter).AlignItems(engine.AlignCenter)
			}
		}
		if rowVisible {
			weekRows = append(weekRows, widgets.Row(dayCells...).AlignItems(engine.AlignCenter).Gapf(2))
		}
	}

	children := make([]engine.Widget, 0, len(weekRows)+2)
