...
func ShadcnPagination(current, total int, onChange func(int)) engine.Widget {
	if total <= 0 {
		return widgets.Row()
	}
	if current < 1 {
	...
	children := make([]engine.Widget, 0, maxVisible+2)
	...
	return widgets.Row(children...).AlignItems(engine.AlignCenter).Gapf(4)
}
