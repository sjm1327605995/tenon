...
// Scroll 创建滚动容器。
func Scroll(child engine.Widget) ScrollWidget {
...
func (s ScrollWidget) CreateElement() engine.Element {
	return engine.NewSingleChildRenderObjectElement(s)
}

// CreateRenderObject implements RenderObjectFactory.
func (s ScrollWidget) CreateRenderObject(element engine.Element) render.RenderObject {
...
func (s ScrollWidget) UpdateRenderObject(ro render.RenderObject, oldWidget engine.Widget) {
	old := oldWidget.(ScrollWidget)
	applyScrollProps(ro.(*render.RenderScroll), old, s)
}

// GetChildWidget implements SingleChildProvider.
func (s ScrollWidget) GetChildWidget() engine.Widget {
	return s.child
}
