...
// Draggable 创建可拖拽包装器。
func Draggable(child engine.Widget, data any) DraggableWidget {
	return DraggableWidget{Child: child, Data: data}
}

func (d DraggableWidget) CreateElement() engine.Element {
...
type DraggableElement struct {
	engine.SingleChildComponentElement
	widget DraggableWidget
}

func (e *DraggableElement) Update(newWidget engine.Widget) {
	e.widget = newWidget.(DraggableWidget)
	e.SingleChildComponentElement.Update(newWidget)
}

func (e *DraggableElement) UpdateChild(oldWidget engine.Widget) {
	e.Child = engine.UpdateChild(e, e.Child, e.widget.Child)
	if ro := e.Child.FindRenderObject(); ro != nil {
		ro.SetDragData(e.widget.Data)
	}
}

func (e *DraggableElement) Mount(parent engine.Element, slot int) {
	e.SingleChildComponentElement.Mount(parent, slot)
	if e.widget.Child != nil {
		e.Child = engine.UpdateChild(e, nil, e.widget.Child)
	}
	if ro := e.Child.FindRenderObject(); ro != nil {
		ro.SetDragData(e.widget.Data)
	}
...
func DropTarget(child engine.Widget, onAccept func(data any)) DropTargetWidget {
	return DropTargetWidget{Child: child, OnAccept: onAccept}
}

func (d DropTargetWidget) CreateElement() engine.Element {
...
func (e *DropTargetElement) Update(newWidget engine.Widget) {
	e.widget = newWidget.(DropTargetWidget)
	e.SingleChildComponentElement.Update(newWidget)
}

func (e *DropTargetElement) UpdateChild(oldWidget engine.Widget) {
	e.Child = engine.UpdateChild(e, e.Child, e.widget.Child)
	if ro := e.Child.FindRenderObject(); ro != nil {
...
func (e *DropTargetElement) Mount(parent engine.Element, slot int) {
	e.SingleChildComponentElement.Mount(parent, slot)
	if e.widget.Child != nil {
		e.Child = engine.UpdateChild(e, nil, e.widget.Child)
	}
