...
func (b *Breadcrumb) buildItems() {
	for i, item := range b.items {
		if i > 0 {
			sep := native.NewText(b.sepText).SetColor(core.GetTheme().MutedForegroundColor)
			sep.SetFontSize(14)
			b.AppendChild(sep)
		}
		var txt *native.Text
		if item.IsActive {
			txt = native.NewText(item.Label).SetColor(core.GetTheme().TextColor)
		} else {
			txt = native.NewText(item.Label).SetColor(core.GetTheme().MutedForegroundColor)
		}
		txt.SetFontSize(14)
		b.AppendChild(txt)
	}
