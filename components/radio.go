...
func NewRadio(label string) *Radio {
	...
	if label != "" {
		r.labelEl = native.NewText(label).SetColor(theme.TextColor)
...
func (r *Radio) Draw(screen *ebiten.Image) {
	...
	if r.selected {
		native.DrawFilledCirclePath(screen, centerX, centerY, r.boxSize/2, r.fillColor)
		native.DrawFilledCirclePath(screen, centerX, centerY, r.boxSize/4, r.innerColor)
	} else {
		native.DrawFilledCirclePath(screen, centerX, centerY, r.boxSize/2, bgClr)
		native.StrokeCirclePath(screen, centerX, centerY, r.boxSize/2, 1.5, r.borderColor)
	}
}
...
func (r *Radio) DebugProps() map[string]interface{} {
	...
	if r.borderColor != nil {
		props["borderColor"] = native.ColorToCSS(r.borderColor)
	}
	if r.selected && r.fillColor != nil {
		props["backgroundColor"] = native.ColorToCSS(r.fillColor)
	}
	if r.innerColor != nil {
		props["innerColor"] = native.ColorToCSS(r.innerColor)
	}
	return props
}
...
func (r *Radio) SyncFrom(src core.Element) {
	...
	sb := &core.SyncBuilder{}
	core.SyncField(sb, &r.selected, other.selected)
	core.SyncField(sb, &r.boxSize, other.boxSize)
	core.SyncColor(sb, &r.borderColor, other.borderColor)
	core.SyncColor(sb, &r.fillColor, other.fillColor)
	core.SyncColor(sb, &r.innerColor, other.innerColor)
	sb.MarkDraw(r)
}
