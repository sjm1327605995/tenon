...
func NewBadge(Content string) *Badge {
	...
	if Content == "" {
		...
		b.textEl = native.NewText(Content).SetColor(theme.PrimaryForegroundColor)
...
func (b *Badge) SyncFrom(src core.Element) {
	...
	sb := &core.SyncBuilder{}
	core.SyncField(sb, &b.dotMode, other.dotMode)
	core.SyncField(sb, &b.maxCount, other.maxCount)
