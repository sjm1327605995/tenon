...
func (s *Switch) Draw(screen *ebiten.Image) {
	...
	// Draw rounded track
	r := bounds.Height / 2
	native.DrawRoundedRectFill(screen, bounds.X, bounds.Y, bounds.Width, bounds.Height, core.BorderRadius{TopLeft: r, TopRight: r, BottomRight: r, BottomLeft: r}, trackColor)
	...
	native.DrawFilledCirclePath(screen, thumbX, thumbY, thumbRadius, s.thumbColor)
}
...
func (s *Switch) DebugProps() map[string]interface{} {
	...
	if trackColor != nil {
		props["backgroundColor"] = native.ColorToCSS(trackColor)
	}
	if s.thumbColor != nil {
		props["thumbColor"] = native.ColorToCSS(s.thumbColor)
	}
...
func (s *Switch) SyncFrom(src core.Element) {
	...
	core.SyncColor(sb, &s.offColor, other.offColor)
	core.SyncColor(sb, &s.onColor, other.onColor)
	core.SyncColor(sb, &s.thumbColor, other.thumbColor)
	sb.MarkDraw(s)
}
