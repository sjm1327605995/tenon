...
func (s *Slider) Draw(screen *ebiten.Image) {
	...
	native.DrawFilledCirclePath(screen, thumbX, trackY, s.thumbRadius, s.thumbColor)
	native.StrokeCirclePath(screen, thumbX, trackY, s.thumbRadius, 1.5, core.GetTheme().BorderColor)
}
...
func (s *Slider) SyncFrom(src core.Element) {
	...
	sb := &core.SyncBuilder{}
	core.SyncField(sb, &s.value, other.value)
	if s.minValue != other.minValue || s.maxValue != other.maxValue {
		s.minValue = other.minValue
		s.maxValue = other.maxValue
		sb.NeedDraw = true
	}
	core.SyncColor(sb, &s.trackColor, other.trackColor)
	core.SyncColor(sb, &s.fillColor, other.fillColor)
	core.SyncColor(sb, &s.thumbColor, other.thumbColor)
	core.SyncField(sb, &s.thumbRadius, other.thumbRadius)
	core.SyncField(sb, &s.trackHeight, other.trackHeight)
	sb.MarkDraw(s)
}
