...
func (pb *ProgressBar) DebugProps() map[string]interface{} {
	...
	if pb.trackColor != nil {
		props["backgroundColor"] = native.ColorToCSS(pb.trackColor)
	}
	if pb.fillColor != nil {
		props["fillColor"] = native.ColorToCSS(pb.fillColor)
	}
...
func (pb *ProgressBar) SyncFrom(src core.Element) {
	...
	sb := &core.SyncBuilder{}
	core.SyncField(sb, &pb.progress, other.progress)
	core.SyncColor(sb, &pb.trackColor, other.trackColor)
	core.SyncColor(sb, &pb.fillColor, other.fillColor)
	core.SyncField(sb, &pb.BorderRadius, other.BorderRadius)
	sb.MarkDraw(pb)
}
