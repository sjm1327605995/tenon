...
func TestText_ChainAPI(t *testing.T) {
	txt := native.NewText("hello")
	if txt.SetContent("world") != txt {
		t.Error("SetContent should return *native.Text")
	}
	if txt.SetFontSize(20) != txt {
		t.Error("SetFontSize should return *native.Text")
	}
	if txt.SetColor(nil) != txt {
		t.Error("SetColor should return *native.Text")
	}
	if txt.SetWhiteSpace(native.WhiteSpaceNormal) != txt {
		t.Error("SetWhiteSpace should return *native.Text")
	}
	if txt.SetWordBreak(native.WordBreakNormal) != txt {
		t.Error("SetWordBreak should return *native.Text")
	}
	if txt.SetLineHeight(24) != txt {
		t.Error("SetLineHeight should return *native.Text")
	}
}
...
func TestScrollView_MaxScrollY(t *testing.T) {
	...
	sv.SetWidth(200)
	sv.SetHeight(100)
	child := native.NewView().SetWidth(50).SetHeight(50)
...
func TestScrollView_ScrollBounds(t *testing.T) {
	...
	sv.SetWidth(200)
	sv.SetHeight(100)
	child := native.NewView().SetWidth(50).SetHeight(300)
	sv.Content().AppendChild(child)
