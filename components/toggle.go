...
func NewToggle(label string) *Toggle {
	...
	t.labelEl = native.NewText(label).SetColor(t.textColor)
...
func (t *Toggle) Draw(screen *ebiten.Image) {
	...
	if bg != nil {
		native.DrawRoundedRectFill(screen, bounds.X, bounds.Y, bounds.Width, bounds.Height, br, bg)
	}
	if t.variant == ToggleOutline || t.pressed {
		...
		if borderClr != nil {
			native.DrawRoundedRectStroke(screen, bounds.X, bounds.Y, bounds.Width, bounds.Height, br, 1, borderClr)
		}
	}
}
