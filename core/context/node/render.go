...
func DrawImage(screen *ebiten.Image, startX, startY float32, node *yoga.Node, img image.Image) error {
...
func DrawRectangle(screen *ebiten.Image, startX, startY float32, node *yoga.Node, radius Radius, color color.RGBA) error {
	//包含了border的宽度
	w, h := node.StyleGetWidth(), node.StyleGetHeight()
	if w == 0 || h == 0 {
		return nil
	...
	if radius.TopLeft > 0 || radius.TopRight > 0 || radius.BottomRight > 0 || radius.BottomLeft > 0 {
		renderFillRoundedRect(screen, w, h, startX, startY, radius, color)
	} else {
...
func renderFillRoundedRect(screen *ebiten.Image, w, h, x, y float32, cornerRadius Radius, _color color.RGBA) {
	// 颜色归一化（float32）
